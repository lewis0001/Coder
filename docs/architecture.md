...
## Module boundaries
- **Auth**: guards, strategies, and DTO validation live in `apps/api/src/auth`; reusable decorators enforce roles across feature modules.
- **Domains**: food, shop, box, wallet, courier, and partner live in their own NestJS modules, each exporting controllers + services that depend on Prisma repositories.
- **Cross-cutting**: notifications, realtime, and payments are pluggable modules injected where needed (wallet top-ups enqueue notifications and emit realtime updates).
- **Web apps**: admin and partner apps rely on API JWTs stored in signed cookies; middleware inspects session cookies and redirects based on role.
- **Mobile**: uses Expo with shared design tokens from `@orbit/ui`, shared realtime helpers from `@orbit/shared`, and localized copy via `i18n`.

## Data flow highlights
- **Order lifecycle**: API validates and persists orders, emits realtime updates, and records wallet debits/credits; admin dashboards subscribe to task/order rooms via Socket.IO.
- **Courier shift**: mobile toggles online status → API records shift + heartbeat location → admin dashboard sees task availability + status updates.
- **Payments**: wallet top-up requests create Stripe PaymentIntents; webhook updates ledger entries and notifies clients; retries are idempotent via PaymentIntent IDs.
- **Files**: proof-of-delivery uploads are stored in MinIO/S3 bucket with object keys stored in the DB; signed URLs can be generated by the API as needed.
